...
def generate_markdown_file(title: str, summary: List[str], categories: List[Dict], page_url: str, md_path: str):
    """
    生成 Markdown 文件用于本地阅读
    """
    content = f"""# {title}
    ...
    if categories:
        ...
        for cat in categories:
            ...
            for item in items:
                ...
                if item_summary:
                    content += f"{item_summary}\n\n"
                if item_url:
                    content += f"🔗 [原文链接]({item_url})\n\n"
                content += "---\n\n"
...
"""
    ...
    log(f"Markdown 文件已生成: {md_path}", "SUCCESS")
...
def send_notification(title: str, slug: str, summary: List[str], categories: List[Dict], html_path: str):
    """发送通知到 TG 和飞书"""
    ...
    # 生成 Markdown 文件（用于本地阅读）
    md_path = html_path.replace(".html", ".md")
    generate_markdown_file(title, summary, categories, page_url, md_path)
    ...
    if result.returncode == 0:
        log("已发送 HTML 链接", "SUCCESS")
    
    # 2. 发送 Markdown 文件（用于本地阅读）
    log("发送 Markdown 文件...", "STEP")
    
    url = f"https://api.telegram.org/bot{TG_BOT_TOKEN}/sendDocument"
    
    caption = f"📄 {title}\n\nMarkdown 源文件，可用于本地阅读"
    
    cmd = [
        "curl", "-s", "-X", "POST",
        "-F", f"chat_id={TG_CHAT_ID}",
        "-F", f"document=@{md_path}",
        "-F", f"caption={caption}",
        url
    ]
...
def main():
    ...
    try:
        ...
        send_notification(title, slug, summary, categories, html_path)
